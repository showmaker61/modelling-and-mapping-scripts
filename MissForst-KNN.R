...
packages <- c("dplyr", "caret", "RANN", "missForest")
...
hyper_grid <- expand.grid(
 k = c(15, 25, 35, 45, 60, 80, 100, 120, 150) 
)
...
for(j in 1:nrow(hyper_grid)) {
  ...
  for(i in 1:n_folds) {
    ...
    p_tr <- predict(knn_model, x_tr_scaled)
    p_val <- predict(knn_model, x_val_scaled)
  ...
  if(j %% 1 == 0) {
    cat(sprintf("组合 %d (k=%d) | Val: %.4f | Train: %.4f | Gap: %.3f\n", 
                j, c_k, grid_results$Val_R2[j], grid_results$Train_R2[j], 
                grid_results$Train_R2[j] - grid_results$Val_R2[j]))
  }
}
...
f_preds <- predict(final_knn, x_te_final_scaled)
f_preds_train <- predict(final_knn, x_tr_final_scaled)
