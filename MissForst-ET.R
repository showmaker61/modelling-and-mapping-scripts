...
packages <- c("dplyr", "caret", "ranger", "RANN", "missForest")
...
hyper_grid <- expand.grid(
  ...
  num.trees = c(500, 800, 1000),
  sample.fraction = c(0.7, 0.8, 1.0) 
)
...
for(j in 1:nrow(hyper_grid)) {
  ...
  if(j %% 2 == 0 || j == 1) {
    ...
    cat(sprintf("组合 %2d (mtry:%d, node:%d) | Val: %.4f | Train: %.4f | Gap: %.3f\n", 
                j, mt, ns, grid_results$Val_R2[j], grid_results$Train_R2[j], gap))
  }
}
