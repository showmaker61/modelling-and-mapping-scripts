packages <- c("terra", "sf", "ranger", "ggplot2", "viridis", 
              "dplyr", "geodata", "scales", "ggspatial")
...
soil_attributes <- soil_data_raw[, c("MU_GLOBAL", "OC", "pH", "Clay", "CEC")]
...
plot_soil_map <- function(plot_data, sf_data, scale_style = "ticks", 
                          title_text = "***", border_width = 1.5) {
  
  p <- ggplot() +
    geom_tile(data = plot_data, aes(x = x, y = y, fill = RAI), color = NA) +
    geom_sf(data = sf_data, fill = NA, color = "black", linewidth = 0.5) +
