...
packages <- c("dplyr", "caret", "xgboost", "missForest")
...
for(j in 1:nrow(hyper_grid)) {
  ...
  for(i in 1:n_folds) {
    ...
    mf_obj <- missForest(combined_x, ntree = 100, maxiter = 10, verbose = FALSE)
    ...
    m <- xgb.train(params = params, data = dtr, nrounds = current_nrounds, verbose = 0)
  ...
  if(j %% 1 == 0) {
    ...
    cat(sprintf("组合 %2d (Rnds:%d) | Val: %.4f | Gap: %.3f\n", 
                j, current_nrounds,
...
mf_final <- missForest(combined_final, ntree = 100, maxiter = 10, verbose = FALSE)
...
cat("CV Val R²       :", round(best_row$Val_R2, 4), "\n")
