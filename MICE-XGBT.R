...
packages <- c("dplyr", "caret", "xgboost", "mice")
...
hyper_grid <- expand.grid(
  ...
  subsample = c(0.5, 0.55, 0.6, 0.7, 0.8, 0.9),
  alpha = c(0, 0.1, 1, 3, 5), 
...
for(j in 1:nrow(hyper_grid)) {
  ...
  for(i in 1:n_folds) {
    ...
    n_tr_rows <- nrow(x_tr)
    combined_x <- rbind(x_tr, x_val)
    
    mice_out <- mice(combined_x, method = "pmm", m = 1, maxit = 5, printFlag = FALSE, remove.collinear = FALSE)
    combined_imp <- complete(mice_out, 1) 
  ...
  if(j %% 1 == 0) {
    ...
    cat(sprintf("组合 %d/%d (Rnds:%d) | Val: %.4f | Gap: %.3f\n", 
                j, nrow(hyper_grid), current_nrounds,
                grid_results$Val_R2[j], gap))
  }
}
...
n_full_rows <- nrow(x_full)
combined_final_x <- rbind(x_full, x_test)

final_mice <- mice(combined_final_x, method = "pmm", m = 1, maxit = 5, printFlag = FALSE, remove.collinear = FALSE)
