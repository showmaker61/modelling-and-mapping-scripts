...
packages <- c("dplyr", "caret", "ranger", "RANN")
...
final_imputer <- preProcess(x_full, method = c("nzv", "knnImpute"), k = 5)

x_tr_final <- predict(final_imputer, x_full)
x_te_final <- predict(final_imputer, x_test)
